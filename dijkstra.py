...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        """
#        assign each node value infinity
        heap = PriorityQueue()
        for node in nodes:
            node.parent = None
            node.queueKey = None
#            ndPair = NodeDistancePair(node,float("inf"))
#            heap.insert(ndPair)
...
#        parent[source] = None
#        sourceND = heap[source]
#        sourceND.distance = 0
#        heap[source] = sourceND
#        heap.decrease_key(sourceND)
        
        source.queueKey = NodeDistancePair(source,0)
        heap.insert(source.queueKey)
        
        while len(heap) > 0:
            key = heap.extract_min()
            keynode, keydist = key.node, key.distance
#            visited.append(u)
            if keynode == destination:
#                destFound = True
                break
            for loc in keynode.adj:
                newDist = keydist + weight(keynode,loc)
                if loc.queueKey is None:
                    loc.parent = keynode
                    loc.queueKey = NodeDistancePair(loc,newDist)
                    heap.insert(loc.queueKey)
                elif newDist < loc.queueKey.distance:
                    loc.parent = keynode
                    loc.queueKey.distance = newDist
                    heap.decrease_key(loc.queueKey)
...
#            resultPath.append(parent[pos])
#            pos = parent[pos]
#        resultPath.reverse()
#        resultLen = len(resultPath)
#        return tuple(resultPath,resultLen)
