...
class PriorityQueue:
    ...
    def verify_min_heap(self):
        """Not a complete check, just check the min is the first elem"""
        min = self.queue[0]
        for val in self.queue:
            if val < min:
                raise ValueError('min heap is not valid')
    
    def min_heapify_up(self, index):
        """sorts new element in heap"""
#        if index == 0:
#            return
...
#            self.min_heapify_up(parent_index)
    ...
    def min_heapify_down(self, index):
        ...
        while True:
            ...
            array_length = len(self.queue)-1
#            if array_length < left_child_index:
#                break
                              
            if  left_child_index <= array_length and self.queue[left_child_index] < self.queue[index]:
                min = left_child_index
            if  right_child_index <= array_length and self.queue[right_child_index] < self.queue[min]:
                min = right_child_index
            if min != index:
                self.queue[index], self.queue[min] = self.queue[min], self.queue[index]
                index = min
            else:
                break
