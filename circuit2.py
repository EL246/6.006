...
class RangeIndex(object):
  ...
  def add(self, key):
    ...
    self.data.insert(self.binary_search(key), key)
...
#    return [key for key in self.data if first_key <= key <= last_key]
    result = []
    low_index = self.binary_search(first_key)
    high_index = self.binary_search(last_key)
...
#    if low_index >= 0 and self.data[low_index] == first_key:
#        count = count + 1
    if high_index < len(self.data) and self.data[high_index] == last_key:
        count = count + 1
    return count
